...
//! Hamming(72,64) ECC + AVX-512 interlock + post-quench recovery) into a
//! static library for the Rust FFI surface.  The `build-kernel` CLI command
//! produces the dynamic interface library `bin/shbt_reference.so` separately
//! via kernel/Makefile.
